...
const findTaskById = (id) => dataStore.find((task) => task.id === id);
...
app.post("/cards/user/:userId", (req, res) => {
	...
	if (title === undefined || completed === undefined) {
		return res.status(400).json({
			error: "Missing title or completed field in request body",
		});
	}
	const newTask = {
...
app.delete("/cards/:id", (req, res) => {
	const id = parseInt(req.params.id, 10);
	const index = dataStore.findIndex((task) => task.id === id);
